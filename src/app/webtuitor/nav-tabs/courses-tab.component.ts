...
@Component({
    ...
    <div class="card-group">
        <div [routerLink]="['/learn/courses/',course.id]" *ngFor="let course of courses | slice:0:4; let i=index" class="card">
        <img height="200" class="card-img-top" [src]="course.imageUrl" alt="Card image cap">
        <div class="card-body">
            <h5 class="card-title">{{capitalizeFirstLetter(course.title)}}</h5>
            <p class="card-text">{{trimDesc(course.description, 100)}}</p>
            <button [routerLink]="['/learn/courses/',course.id]" class="btn btn-md btn-outline-info">Start</button>
            <p class="card-text">
